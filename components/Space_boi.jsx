...
Command: npx gltfjsx@6.2.14 space_boi.glb 
...
export function SpaceModel(props) {
  ...
  const rotateY = useTransform(xAxis, [0, 3000], [-0.9, 0], {
    ease: easeInOut,
  });
  const rotateX = useTransform(yAxis, [0, 3000], [0.45, 1], {
    ease: easeInOut,
  });

  useFrame(({ clock }) => {
    const t = clock.getElapsedTime();
    // myMesh.current.rotation.x = Math.PI / 5 + Math.cos(t / 4) / 8;
    // myMesh.current.rotation.y = Math.sin(t / 4) / 7;
    myMesh.current.rotation.z = (1 + Math.sin(t / 1.5)) / 15;
    myMesh.current.position.y = (1 + Math.sin(t / 1.5)) / 15;
  });
